      ...
      <v-btn-toggle v-model="toggle_exclusive" mandatory>
        <v-col class="text-center" v-for="(clss, i) in classList" :key="clss.name" style="background-color:rgb(245, 245, 247, 0.7)">
          <v-btn active-class="no-active" text @click="$store.state.poomsaeCurNo=i+1; $router.push({name: 'Training'}); console.log('뭐야');" style="outline:none">
            <v-img v-if="$store.state.poomsaeCurNo==i+1" height="50px" width="50px" :src="clss.imgPath2"></v-img>
            <v-img v-else height="50px" width="50px" :src="clss.imgPath"></v-img>
          </v-btn>
          <div v-if="$store.state.poomsaeCurNo==i+1" style="color:rgb(0, 112, 201)" class="text-caption">{{ clss.name }}</div>
